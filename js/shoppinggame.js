...
// Here, use Object.defineProperty to create property - daysToExpire The Product() object type should have a property named daysToExpire that holds the number of days a product has, to reach its expiry date. To add this new property to the already defined Product() object type, use the Object.defineProperty() method, passing the prototype of Product. Add a getter function to the daysToExpire property that should return the number of days to expire. For now, let's do nothing inside the getter function. Hint: You should use the prototype of Product in Object.defineProperty() method.
...
// Define the MagicProduct class here -- Another object type named MagicProduct() should be added which would be a child of Product(). This new object type should have two more properties, points and isBonus in addition to the properties of Product. To do this, define a constructor function named MagicProduct(). The function should have parameters id, name, price, expiryDate, points, and isBonus passed to it in the given order. In the body of the function, invoke the constructor function of Product() using the [call() method] (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/call). Pass this as the 1st argument and id, name, price, and expiryDate as the subsequent arguments in the given order. This should be the first statement inside the constructor function body. Below this statement, you need to initialize the points and isBonus properties.
...
// Establish inheritance between Product() & MagicProduct() here -- To do this, call the Object.create() method on the prototype of Product(), and assign the result of the method call to MagicProduct.prototype. You should do this right below MagicProduct() declaration.
...
// Complete the loadProducts function -- Inside the loadProducts() function, we need to set the value of prodKeys so that it holds an array of the property names of the Product() object type. This should be achieved by calling the Object.keys() method. The return value from the Object.keys() method call, which is an array, should be assigned to prodKeys.
...
// Complete this function -- The findProductById() function is called inside getProduct(), and findProductById() locates a product by the given id within the array of products. We need to add code to the findProductById() function which returns an anonymous function. To achieve this, inside the findProductById() function, return an anonymous function that takes in an instance of Product(). From within the anonymous function, return true if the id parameter is equal to the id property of the Product() instance, false otherwise.
...
// Complete this function -- In the generateProductId() function, generate and return a random integer between 1 and 20 (including 1 and 20). This integer will be the product ID that we are looking up in the array of products. You will see this function in action in module 6, when we implement the shopping functionality of the game. To achieve this, use the Math.random() method and another suitable function from the Math built-in object.
